...
namespace baseTemplate
{
    public class SoundManager : MonoBehaviour
    {
        ...
        private const int MAX_KEYS_FOR_DELETE = 100;
        private int[] keysForDelete = new int[MAX_KEYS_FOR_DELETE];
        ...
        void Update()
        {
            ...
            //keys = new List<int>(musicAudio.Keys);
            int countDelete = 0;
            foreach (var curAudio in musicAudio.Values)
            {
                //Audio audio = musicAudio[key];
                curAudio.Update();

                // Remove all music clips that are not playing
                if (!curAudio.playing && !curAudio.paused)
                {
                    Destroy(curAudio.audioSource);
                    //musicAudio.Remove(key);
                    if (countDelete < MAX_KEYS_FOR_DELETE)
                    {
                        keysForDelete[countDelete++] = curAudio.audioID;
                    }
                }
            }
            ...
            {
                musicAudio.Remove(keysForDelete[i]);
            }

            // Update sound fx
            //keys = new List<int>(soundsAudio.Keys);
            countDelete = 0;
            foreach (var curAudio in soundsAudio.Values)
            {
                curAudio.Update();
            ...
            for (int i = 0; i < countDelete; i++)
            {
                soundsAudio.Remove(keysForDelete[i]);
            }

            // Update UI sound fx
            //keys = new List<int>(UISoundsAudio.Keys);
            countDelete = 0;
            foreach (var curAudio in UISoundsAudio.Values)
            {
                curAudio.Update();
            ...
            for (int i = 0; i < countDelete; i++)
            {
                UISoundsAudio.Remove(keysForDelete[i]);
            }
        }
        ...
        {
            if (musicAudio.ContainsKey(id)) musicAudio[id].Pause();
        }
        ...
        {
            if (soundsAudio.ContainsKey(id)) soundsAudio[id].Pause();
        }
        ...
        public static void PauseUISound(int id)
        {
            if (UISoundsAudio.ContainsKey(id)) UISoundsAudio[id].Pause();
        }
        #endregion
        ...
        {
            if (musicAudio.ContainsKey(id)) musicAudio[id].Resume();
        }

        public static void ResumeSound(int id)
        {
            if (soundsAudio.ContainsKey(id)) soundsAudio[id].Resume();
        }

        public static void ResumeUISound(int id)
        {
            if (UISoundsAudio.ContainsKey(id)) UISoundsAudio[id].Resume();
        }
