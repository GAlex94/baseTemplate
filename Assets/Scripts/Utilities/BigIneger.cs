...
namespace Numerics
{
    ...
    [Serializable]
    public class UnityBigInteger
    {
        [SerializeField]
        public string cachedString = "0";
        private BigInteger bigInt = new BigInteger();
        public BigInteger Value
        {
            get
            {
                if (bigInt.cachedString != cachedString)
                {
                    bigInt.cachedString = cachedString;
                    bigInt.UpgradeDigits();
                }
                return bigInt;
            }
            set
        ...
        public void UpdateCachedString()
        {
            cachedString = bigInt.UpdateCachedString();
        }

        public UnityBigInteger()
        {
            bigInt = new BigInteger(cachedString);
        }

        public void ParseString(string digits)
        {
            cachedString = digits;
            bigInt = new BigInteger(cachedString);
        }

        public UnityBigInteger(string digits)
        {
            ParseString(digits);
        }

        public UnityBigInteger(BigInteger val)
        {
            ParseString(val.ToString());
        }

        public UnityBigInteger Pow(BigInteger power)
        {
            if (power == 0)
                return 1;
            ...
            return new UnityBigInteger(bigInt);
        }
        ...
        // public static UnityBigInteger operator = (int x)
        // {
        // 	return new UnityBigInteger(x.ToString());
        // }

        public static bool operator <=(UnityBigInteger leftSide, UnityBigInteger rightSide)
        {
        ...
        public static UnityBigInteger operator /(UnityBigInteger leftSide, UnityBigInteger rightSide)
        {
            UnityBigInteger temp = new UnityBigInteger();
            temp.Value = leftSide.Value / rightSide.Value;
            return temp;
        }
        ...
        public static UnityBigInteger operator -(UnityBigInteger leftSide)
        {
            UnityBigInteger temp = new UnityBigInteger();
            temp.Value = -leftSide.Value;
            return new UnityBigInteger();
        }

        ///for double

        public static UnityBigInteger operator *(UnityBigInteger leftSide, double rightSide)
        {
            int mult = 10000; // точность

            UnityBigInteger temp = new UnityBigInteger();

            BigInteger decimalBig = new BigInteger((Math.Truncate(rightSide * mult)).ToString()) * leftSide.Value / mult;
            temp.Value = decimalBig;
        ...
        public static UnityBigInteger operator /(UnityBigInteger leftSide, double rightSide)
        {
            ...
            BigInteger decimalBig = ((leftSide.Value * mult) / new BigInteger((rightSide * mult).ToString()));
            temp.Value = temp.Value + decimalBig;
        ...
        //// for int
        public static bool operator ==(UnityBigInteger leftSide, int rightSide)
        {
        ...
        public static UnityBigInteger operator /(UnityBigInteger leftSide, int rightSide)
        {
            UnityBigInteger temp = new UnityBigInteger();
            temp.Value = leftSide.Value / rightSide;
            return temp;
        }
        ...
        // public static explicit operator UnityBigInteger(int rightSide)
        // {
        // 	return new UnityBigInteger(rightSide);
        // }

        public static implicit operator UnityBigInteger(int rightSide)
        {
            return new UnityBigInteger(rightSide);
        }
    ...
    public class BigInteger
    {
        ...
        public void UpgradeDigits()
        {
            m_digits = new BigInteger(cachedString).m_digits;
        }

        /// <summary>
		/// Create a BigInteger with an integer value of exp(e).
		/// </summary>
		public static BigInteger FromExp(double e)
        {
            double x = e / 2.3025850929940456840179914546844;

            int n = (int)x;
            double f = x - n;

            double f2 = System.Math.Pow(10, f);

            string digits = "1";
            for (int i = 0; i < n; i++) digits += "0";

            BigInteger res = new BigInteger(digits);

            //Debug.Log(e + " -> " + digits + " " + x + " " + n + " " + f.ToString("F10") + " " + f2.ToString("F10") + " | " + res);
        ...
        private List<char> letters = new List<char>("ABCDEFGHIJKLMNOPQRSTUVWXYZ".ToCharArray());
        private int oneLetterCost = 3;
        private int basePow = 12;
        ...
        {
            if (log) Debug.Log("Initialize big number " + digits);

            if (!string.IsNullOrEmpty(digits))
            {
                digits = digits.ToUpper(System.Globalization.CultureInfo.CurrentCulture).Trim();

                if (log) Debug.Log("Digits is " + digits);

                int ind = -1;
                for (int i = 0; i < digits.Length; i++)
                {
                    if (letters.Contains(digits[i]))
                    {
                        ind = i;
                        break;
                    }
                ...
                if (log) Debug.Log("Found letter at " + ind);

                if (ind > 0)
                {
                    string sNumber = digits.Substring(0, ind);

                    if (log) Debug.Log("sNumber " + sNumber);

                    float fNumber = 0;

                    if (!float.TryParse(sNumber, out fNumber))
                    {
                        Construct(digits, 10);
                        return;
                    }

                    if (log) Debug.Log("fNumber " + fNumber);

                    int pointPos = sNumber.IndexOf(".");

                    int add = 0;
                    if (pointPos >= 0) add = sNumber.Length - 1 - pointPos;

                    fNumber *= (float)Math.Pow(10, add);

                    if (log) Debug.Log("fNumber after " + fNumber);

                    int iNumber = (int)(fNumber + 0.001f);

                    if (log) Debug.Log("iNumber " + iNumber);

                    if (log) Debug.Log("Add " + add);

                    string postfix = "";
                    for (int i = ind; i < digits.Length; i++)
                    {
                        if (letters.Contains(digits[i])) postfix += digits[i];
                        else break;
                    }

                    if (log) Debug.Log("Postfix " + postfix);
                    ...
                    if (!found)
                    {
                        ...
                        {
                            int letter = letters.IndexOf(postfix[i]) + shift;
                            pow += letter * p;
                            p *= letters.Count;
                            shift = 0;
                        }
                    ...
                    for (int i = 0; i < pow; i++) digits += "0";

                    if (log) Debug.Log("Initialize " + pow);
        ...
        public static implicit operator int(BigInteger value)
        {
        ...
        private string postfixes = "KMBTQX";
        ...
        public string Short()
        {
            ...
            //Debug.Log(this + " pow is " + pow + " (" + (basePow + oneLetterCost) + ")");

            BigInteger a = 1;
            for (int i = 0; i < pow - (oneLetterCost - 1); i++) a *= 10;

            string postfix = "";

            if (pow < basePow + oneLetterCost)
            {
                if (pow < 3) return str;
                ...
                int _pow = pow;
                int ind = 0;
                while (_pow >= 3 && ind < postfixes.Length)
                {
                    postfix = "" + postfixes[ind];
                    _pow -= 3;
                    ind++;
                }

                //Debug.Log(postfix + " " + _pow);

                ind = 0;
                if (str[0] == '-') ind = 1;

                string res = "";
                for (int i = ind; i < ind + _pow + 1; i++) res += str[i];

                //Debug.Log(res + postfix);

                if (_pow < 2) res += ".";
                //Debug.Log((ind + _pow + 1) + " " + (ind + _pow + 1 + (2 - _pow)));
                for (int i = ind + _pow + 1; i < ind + _pow + 1 + (2 - _pow); i++) res += str[i];
            ...
            while (p < pow) p *= letters.Count;
            if (p >= oneLetterCost * letters.Count) p /= letters.Count;

            int start = 0;
            if (p >= oneLetterCost * letters.Count) start = 1;
            ...
            while (pow > 0 && cnt-- > 0)
            {
                ...
                //Debug.Log("Stop at " + letter + " " + pow);

                postfix += letter;

                p /= letters.Count;
                if (p < oneLetterCost * letters.Count) start = 0;

                //Debug.Log("Continue " + p);
                if (p == 0) break;
            }

            while (postfix.Length < 2) postfix = "A" + postfix;

            {
                BigInteger b = 1;
                for (int i = 0; i < oneLetterCost - 1 - pow; i++) b *= 10;
