...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        public bool isInversLeft;
        public bool isInversRight;
        public bool isInversTop;
        public bool isInversBotton;
        ...
        [SerializeField] bool isNeedBanner = false;
        ...
        void ApplySafeArea(Rect r)
        {
            if (Logging)
            {
                Debug.LogFormat("Old safe area applied to {0}: x={1}, y={2}, w={3}, h={4} on full extents w={5}, h={6}",
            ...
            if (isInversTop || isInversBotton)
            {
                if (isInversTop)
                {
                    anchorMin.y = Panel.anchorMax.y + Panel.offsetMax.y / Screen.height;
                    anchorMax.y = 1;
                }
                else
                {
                    anchorMin.y = 0;
                    anchorMax.y = Panel.anchorMin.y + Panel.offsetMin.y / Screen.height;
                }

                Panel.anchorMin = anchorMin;
                Panel.anchorMax = anchorMax;

                if (isInversBotton && ConformY)
                {
                    UpdateBannerZone.Instance.AddSafeAreaZone(Panel.rect.height);
                }
            }

            if (isInversLeft || isInversRight)
            {
                if (isInversLeft)
                {
                    anchorMin.x = 0;
                    anchorMax.x = Panel.anchorMin.x + Panel.offsetMin.x / Screen.width;
                }
                else
                {
                    anchorMin.x = Panel.anchorMax.x + Panel.offsetMax.x / Screen.width;
                    anchorMax.x = 1;
                }
